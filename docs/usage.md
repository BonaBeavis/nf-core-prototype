# nf-core/prototype: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/prototype/usage](https://nf-co.re/prototype/usage)
...
```console
nextflow run nf-core/prototype --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/prototype
```
...
First, go to the [nf-core/prototype releases page](https://github.com/nf-core/prototype/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
